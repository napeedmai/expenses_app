--------------------------------------------------------------------------------
-- 59_email_actor_role_fix.sql
--
-- Run as the application schema, after 56-58. Idempotent.
--
--
-- BUG 1 -- WRONG ROLE IN THE EMAIL
-- --------------------------------
-- A project manager revised a claim and the email said "(FM)", filing his
-- remark under "Fin.Manager Remarks".
--
-- Cause: send_expense_mail decided the role with is_finance_manager(actor).
-- That answers "is this person the finance manager" -- NOT "which role were
-- they acting in just now". When one person holds both roles, as here, the two
-- answers differ and the code took the wrong one.
--
-- It also cannot be recovered from current_stage after the fact:
-- process_expense_action has already moved the stage by the time the mail is
-- sent (to FINANCE on manager-accept, to NULL on final approval).
--
-- Fix: process_expense_action already calls get_reviewer_role BEFORE touching
-- anything -- that value is the truth, and it now gets passed in as
-- p_actor_role. Where one person is both PM and FM, the STAGE decides, which
-- is the only thing that can.
--
--
-- BUG 2 -- EMPTY Employee/Manager/Fin.Mgr ROWS
-- --------------------------------------------
-- Those three cells were blank while every other row rendered. All three come
-- from the same lookup, and a blank rather than the '-' placeholder means the
-- value was not NULL but whitespace -- EMPLOYEEDETAILS name columns are CHAR,
-- so an empty name yields a string of spaces, which NVL happily passes
-- through and HTML then collapses to nothing.
--
-- Fix: TRIM and NULLIF everywhere, so blank becomes '-'; and if the lookup
-- finds no row at all, print "Employee #3725 (not found)" instead of nothing.
-- A blank cell reads as a broken template; the id points at the real problem.
--
-- Section 3 checks the underlying data either way.
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- 1. send_expense_mail -- takes the role from the caller.
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- 2. process_expense_action -- passes get_reviewer_role's answer.
--
--    Verbatim from PROD_3_business_logic.sql; the only change is l_role added
--    to each of the four send_expense_mail calls.
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- 3. Is the underlying data actually there?
--
--    Run this for the expense whose email had the blank rows. If ECODE or the
--    names come back empty here, the email was telling the truth and the fix
--    is in EMPLOYEEDETAILS, not in the template.
--------------------------------------------------------------------------------
SELECT e.id                                            AS expense_id,
       e.current_stage,
       e.emp_id,
       emp.ecode                                       AS emp_ecode,
       TRIM(emp.first_name || ' ' || emp.last_name)    AS emp_name,
       emp.company_email                               AS emp_email,
       e.manager_empid,
       mgr.ecode                                       AS mgr_ecode,
       TRIM(mgr.first_name || ' ' || mgr.last_name)    AS mgr_name,
       mgr.company_email                               AS mgr_email,
       e.finance_manager_empid,
       fin.ecode                                       AS fin_ecode,
       TRIM(fin.first_name || ' ' || fin.last_name)    AS fin_name,
       fin.company_email                               AS fin_email
FROM   expenses e
LEFT   JOIN employeedetails emp ON emp.empid = e.emp_id
...
-- Empty MGR columns with a non-null MANAGER_EMPID means the manager has no
-- EMPLOYEEDETAILS row. A NULL MANAGER_EMPID means the claim was submitted
-- while its project had no project manager assigned -- a data problem worth
-- knowing about, since nobody can approve it at the first stage.
...
-- Force each role explicitly and compare the two mails:
--
--   BEGIN
--     send_expense_mail(<id>, 'REVISED', <actor empid>, 'as manager', 'PROJECT_MANAGER');
--     send_expense_mail(<id>, 'REVISED', <actor empid>, 'as finance', 'FINANCE_MANAGER');
--   END;
--   /
--
-- The first must say (PM) and "Manager Remarks", the second (FM) and
-- "Fin.Manager Remarks". Real actions get the role from get_reviewer_role, so
-- the stage decides -- which is what makes it correct when one person is both.
--
